...
def is_abundant?(num)
  proper_divisors(num).sum > num
end

def abundants_in_range(n)
  (1..n).to_a.select{ |num| is_abundant?(num) }
end

abundants_orig = abundants_in_range( 28123 )

abundants = abundants_orig.dup
a = *(1..28123)
while abundants.length > 0 do
  puts "#{abundants.length}: #{abundants.first}"
  abundant = abundants[0]
  abundants.each{ |ab| a.delete(abundant + ab) }
  abundants.shift
end
a.sum
